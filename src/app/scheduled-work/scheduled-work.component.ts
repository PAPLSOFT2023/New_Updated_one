...
[x: string]: any;
  ...
  isSendMailEnabled(scheduleFrom: string): boolean {
    ...
    const isEnabled = scheduleDate <= this.currentDate;

    console.log('Mail Sending Enabled:', isEnabled);
    return isEnabled;
  }

  isButtonDisabled(scheduleFrom: string): boolean {
    return !this.isSendMailEnabled(scheduleFrom);
  }
  ...
  getRecordCount1(name: string) {
    ...
    this.http.get<any>('http://localhost:3000/api/countRecords22', { params })
      .subscribe(
        count => {
            this.records = count;
        },

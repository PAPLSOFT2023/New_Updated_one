...
export class ViewofcertificateComponent {
  ...
  ngOnInit(){
    ...
    const inspector = `http://localhost:3000/api/b_spec?encodedValue=${this.document_id}&encodedValue1=${this.unit}`;
  ...
  getNextId(): void {
    this.http.get<number>('http://localhost:3000/api/next-id').subscribe(
        (id: number) => {
            this.nextId = id;
        },
        (error) => {
  ...
  fetchSignature() {
    ...
    this.http.get('http://localhost:3000/signature/' + name, { responseType: 'blob' })
      .subscribe((data: Blob) => {
